"==============================================================================
" CONFIGURAÇÃO NEOVIM FINAL - CERBERO - (VERSÃO M3 MAX CONSOLIDADA)
"==============================================================================
...
" --- AMBIENTE COBOL (Lógica, Compilação e Wrap Físico) ---
...
function! RodarCobol()
    ...
    " Aspas simples garantem que caminhos com espaços no macOS funcionem
...
function! CreateCobolTemplate()
    if line('$') == 1 && getline(1) == ''
        ...
        call append(0, [
            ...
            \ "           STOP RUN.", 
            \ "       END PROGRAM " . l:prog . "."])
        " Posiciona o cursor dentro da Procedure Division
        call cursor(12, 11)
    endif
endfunction
autocmd BufNewFile *.cob,*.cbl call CreateCobolTemplate()

" --- AUTOMAÇÃO: EVOLUÇÃO MÉDICA ---
function! GerarEvolucaoMedica()
    ...
    " Tratamento de strings para compatibilidade com o sistema de arquivos do Mac
    ...
    if filereadable(l:arquivo_txt)
        ...
        " Limpeza do arquivo temporário após inserção no buffer
        call delete(l:arquivo_txt)
        echo "\nSucesso! Evolução inserida no documento."
    else
        echo "\nErro ao gerar evolução: " . l:res
    endif
endfunction
...
" Muda a cor para verde no modo de inserção para feedback visual claro
