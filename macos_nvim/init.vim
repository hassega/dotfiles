...
lua << LUA
-- 1. MASON & LSP (Gerenciador de Servidores)
...
-- 3. INTERFACE (Nvim-Tree, Treesitter, Indent-Blankline)
...
if status_ts then
    ts.setup({
        ...
        indent = { enable = true } -- Ativa indentação inteligente do treesitter
    })
end

-- 1. CONFIGURAÇÃO DOS PONTINHOS E LINHA AZUL
...
if status_ibl then
    ...
    ibl.setup {
        indent = {
            ...
            highlight = { "IblIndent" }, -- Usa a cor azul que definimos
        },
        whitespace = {
            highlight = { "IblIndent" }, -- Aplica azul também nos pontinhos
...
if status_tel then
    telescope.setup({
        defaults = {
            preview = {
                treesitter = false, -- Desativa o Treesitter apenas na janela de visualização
            },
        },
    })
end
...
" --- AMBIENTE COBOL (COMPILAÇÃO E WRAP FÍSICO) ---
...
augroup cobol_logic
    autocmd!
    " Unificação: Aplica margem 72, wrap físico (t) e autoindent em um só bloco
    autocmd FileType cobol setlocal textwidth=72 formatoptions+=tc autoindent
    autocmd FileType cobol setlocal wrap linebreak colorcolumn=7,8,12,72
    
    " Configurações para Indentação em Escada (Estilo Python)
...
function! GerarEvolucaoMedica()
    ...
    let l:paciente_slug = substitute(l:paciente, ' ', '_', 'g')
    let l:cmd = "python3 ~/EXAMESPDF/gerar_evolucao.py '" . l:paciente . "'"
    
    echo "\nProcessando..."
    call system(l:cmd)

    let l:arquivo = expand("~/EXAMESPDF/") . l:paciente_slug . "/evolucao_" . l:paciente_slug . ".txt"
    
    if filereadable(l:arquivo)
        execute "read " . l:arquivo
    else
        echo "Erro: Arquivo não gerado. Verifique a pasta do paciente."
    endif
endfunction
